...
class MADRPooler(EncoderPooler):
    def __init__(self, input_dim: int = 768, out_dim: int = 768,
        ...
        # app
        self.brand_fc = nn.Linear(input_dim, 1)
        self.color_fc = nn.Linear(input_dim, 1)
        self.cate_fc = nn.Linear(input_dim, 1)
        self.other_fc = nn.Linear(input_dim, 1)

        self.brand_weight = torch.nn.Parameter(torch.Tensor(1))
        nn.init.constant_(self.brand_weight, 1)
        self.color_weight = torch.nn.Parameter(torch.Tensor(1))
        nn.init.constant_(self.color_weight, 1)
        self.cate_weight = torch.nn.Parameter(torch.Tensor(1))
        nn.init.constant_(self.cate_weight, 1)
        self.other_weight = torch.nn.Parameter(torch.Tensor(1))
        nn.init.constant_(self.other_weight, 1)

        print('============Extra k embeddings(except cls), add other')

        if q_pool_type == 'att':
            self.att_query_q = nn.Parameter(torch.ones(
                self.aspect_num, input_dim).normal_(mean=0.0, std=0.02))  # actual + other
            ...
            self.weight_q = nn.Parameter(torch.ones(
                self.aspect_num, 1).normal_(mean=0.0, std=0.02))
        ...
        if doc_pool_type == 'att':
            if q_pool_type == 'att':  # 对于madr，pooler共享
                ...
                self.weight_d = self.weight_q
            else:
                self.att_query_d = nn.Parameter(torch.ones(
                    self.aspect_num, input_dim).normal_(mean=0.0, std=0.02))
        ...
        # self.gating_type = 'cls-gating:no-cls'
        # self.gating_type = 'cls-gating:cls-as-other'
        # self.gating_type = 'first-k:no-cls'
        # self.gating_type = 'first-k:with-cls'
        # self.gating_type = 'first-k:with-cls-fix'
        # self.gating_type = 'first-k:no-cls-app'
        # self.gating_type = 'first-k:with-cls-app'
        # self.gating_type = 'weighted_sum:no-cls'
        # self.gating_type = 'weighted_sum:with-cls-as-other'
        # self.gating_type = 'app:no-cls'
        # self.gating_type = 'app:with-cls-as-other'
        # print('=========self.gating_type:', self.gating_type)
    ...
    def forward(self, q: Tensor = None, doc: Tensor = None,
        ...
        """
        if 'gating_type' in kwargs:
            gating_type = kwargs.pop('gating_type')
        else:
            gating_type = 'no'

        if 'need_aspect_alpha' in kwargs:
            need_aspect_alpha = kwargs.pop('need_aspect_alpha')
        presence_fc = {
            'p_brand': self.brand_fc,
            'p_color': self.color_fc,
            'p_cate_mul': self.cate_fc,
            'qq_brand': self.brand_fc,
            'qq_color': self.color_fc,
            'qq_cate_mul': self.cate_fc
        }
        gamma_weight = {
            'p_brand': self.brand_weight,
            'p_color': self.color_weight,
            'p_cate_mul': self.cate_weight,
            'qq_brand': self.brand_weight,
            'qq_color': self.color_weight,
            'qq_cate_mul': self.cate_weight
        }
        ...
        if q is not None:
            if self.q_pool_type == 'cls':
                ...
                aspect_emb, _ = self.attn_q(query.unsqueeze(0).repeat(bs, 1, 1).transpose(0, 1),
                                            q_token_seq.transpose(0, 1), q_token_seq.transpose(0, 1),
                                            key_padding_mask=q_token_mask == 0)
                ...
                if gating_type in ['first-k:no-cls', 'first-k:no-cls-app']:
                    begin_id = 1  # 从哪个tokenid开始取，0是cls
                    aspect_emb = q[:, begin_id:begin_id +
                                               self.aspect_num]  # 直接选取 前3个token输出
                elif gating_type in ['first-k:with-cls', 'first-k:with-cls-app']:
                    begin_id = 1  # 从哪个tokenid开始取，0是cls
                    aspect_emb = q[:, begin_id:begin_id +
                                               self.aspect_num - 1]
                    aspect_emb = torch.cat([aspect_emb, q_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'first-k:with-cls-fix':
                    fix_idx = [3, 9, 15]
                    assert len(fix_idx) == self.aspect_num - 1
                    aspect_emb = q[:, fix_idx, :]
                    aspect_emb = torch.cat([aspect_emb, q_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'first-k:no-cls-fix':
                    fix_idx = [3, 9, 15, 7]
                    assert len(fix_idx) == self.aspect_num
                    aspect_emb = q[:, fix_idx, :]

                if gating_type == 'cls-gating:no-cls':
                    pass
                elif gating_type == 'cls-gating:cls-as-other':
                    # ====cls 取代other
                    # [bs, aspect, 768], cls 当作other
                    aspect_emb = torch.cat(
                        [aspect_emb[:, :-1, :], q_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'weighted_sum:no-cls':
                    q_att = torch.softmax(self.weight_q.transpose(
                        0, 1), dim=1).unsqueeze(0).repeat(bs, 1, 1)
                elif gating_type == 'weighted_sum:with-cls-as-other':
                    aspect_emb = torch.cat(
                        [aspect_emb[:, :-1, :], q_cls.unsqueeze(1)], dim=1)
                    q_att = torch.softmax(self.weight_q.transpose(
                        0, 1), dim=1).unsqueeze(0).repeat(bs, 1, 1)

                elif gating_type in ["app:no-cls", 'app:with-cls-as-other', 'first-k:no-cls-app',
                                     'first-k:with-cls-app']:

                    if gating_type == 'app:with-cls-as-other':  # 把other换成cls
                        aspect_emb = torch.cat(
                            [aspect_emb[:, :-1, :], q_cls.unsqueeze(1)], dim=1)

                    presence_weight = torch.zeros(
                        (q.shape[0], self.aspect_num))  # [bs, 3]
                    for aspect_name, alpha in need_aspect_alpha.items():
                        if alpha != 0:
                            presence_weight[:, alpha - 1] = presence_fc[aspect_name](
                                aspect_emb[:, alpha - 1]).squeeze()  # [64, 1]
                    presence_weight[:, -
                                       1] = self.other_fc(aspect_emb[:, -1]).squeeze()

                    # [128, 1, 3]
                    q_att = torch.sigmoid(presence_weight).unsqueeze(
                        1).to(aspect_emb.device)

                    gamma = torch.zeros((self.aspect_num)).to(
                        aspect_emb.device)
                    for aspect_name, alpha in need_aspect_alpha.items():
                        if alpha != 0:
                            gamma[alpha - 1] = gamma_weight[aspect_name]
                    gamma[-1] = self.other_weight
                    gamma = gamma.reshape(1, 1, -1)
                    q_att = q_att * gamma

                    res = torch.bmm(q_att, aspect_emb).squeeze(1)
                    return res, aspect_emb, presence_weight.unsqueeze(1)
            ...
            if self.doc_pool_type == 'cls':
                ...
                aspect_emb, _ = self.attn_d(query.unsqueeze(0).repeat(bs, 1, 1).transpose(0, 1),
                                            doc_token_seq.transpose(0, 1), doc_token_seq.transpose(0, 1),
                                            key_padding_mask=doc_token_mask == 0)
                aspect_emb = aspect_emb.transpose(
                    0, 1)  # [bs, aspect_num, 768]
                ...
                if gating_type in ['first-k:no-cls', 'first-k:no-cls-app']:
                    begin_id = 1  # 从哪个 token id 开始取，0 是cls
                    aspect_emb = doc[:, begin_id:begin_id +
                                                 self.aspect_num]  # 直接选取 前3个token输出
                elif gating_type in ['first-k:with-cls', 'first-k:with-cls-app']:
                    begin_id = 1  # 从哪个tokenid开始取，0是cls
                    aspect_emb = doc[:, begin_id:begin_id +
                                                 self.aspect_num - 1]
                    aspect_emb = torch.cat([aspect_emb, doc_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'first-k:with-cls-fix':

                    fix_idx = [3, 9, 15]
                    assert len(fix_idx) == self.aspect_num - 1
                    aspect_emb = doc[:, fix_idx, :]
                    aspect_emb = torch.cat([aspect_emb, doc_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'first-k:no-cls-fix':
                    fix_idx = [3, 9, 15, 7]
                    assert len(fix_idx) == self.aspect_num
                ...
                if gating_type == 'cls-gating:no-cls':
                    ...
                    # [bs, aspect, 768], cls 当作other
                    aspect_emb = torch.cat(
                        [aspect_emb[:, :-1, :], doc_cls.unsqueeze(1)], dim=1)
                elif gating_type == 'weighted_sum:no-cls':
                    doc_att = torch.softmax(self.weight_d.transpose(
                        0, 1), dim=1).unsqueeze(0).repeat(bs, 1, 1)
                elif gating_type == 'weighted_sum:with-cls-as-other':
                    aspect_emb = torch.cat(
                        [aspect_emb[:, :-1, :], doc_cls.unsqueeze(1)], dim=1)
                ...
                elif gating_type in gating_type in ["app:no-cls", 'app:with-cls-as-other', 'first-k:no-cls-app',
                                                    'first-k:with-cls-app']:
                    ...
                    presence_weight = torch.zeros(
                        (doc.shape[0], self.aspect_num))  # [bs, 3]
                    for aspect_name, alpha in need_aspect_alpha.items():
                        if alpha != 0:
                            presence_weight[:, alpha - 1] = presence_fc[aspect_name](
                                aspect_emb[:, alpha - 1]).squeeze()  # [64, 1]
                    ...
                    # [128, 1, 3]
                    doc_att = torch.sigmoid(presence_weight).unsqueeze(
                        1).to(aspect_emb.device)
...
class MADRModel(EncoderModel):
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None,
        ...
        if self.pooler_type == 'app' and doc_aspect_weight is not None:
                    ...
                    if alpha != 0:
                        ...
                        if doc_aspect_name == 'p_cate_mul':
                            ...
                            doc_acc_dict[doc_aspect_name] = accuracy_score(
                                aspect_id[doc_aspect_name].cpu(),
                                p_reps_for_aspect[doc_aspect_name].argmax(1).cpu().tolist())

                        if self.pooler_type == 'app':
                            aspect_label = aspect_id[doc_aspect_name]
                            if doc_aspect_name == 'p_cate_mul':
                                app_label = (aspect_label.sum(1) != 0).int().float()
                            else:
                                app_label = (aspect_label !=
                                             self.zeroid_dict[doc_aspect_name]).float()

                            doc_app_loss_dict[doc_aspect_name] = self.bce(
                                p_presence_for_aspect[doc_aspect_name], app_label)
