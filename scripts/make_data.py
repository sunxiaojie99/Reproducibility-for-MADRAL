...
#     '/path/attempt-code/downloads/amazon_ori/shopping_queries_dataset_examples.parquet')
# df_products = pd.read_parquet(
#     '/path/attempt-code/downloads/amazon_ori/shopping_queries_dataset_products.parquet')
# df_sources = pd.read_csv(
#     "/path/attempt-code/downloads/amazon_ori/shopping_queries_dataset_sources.csv")
...
def make_corpus():
    ...
    with jsonlines.open("/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus.jsonl", 'w') as w:
...
def makde_corpus_with_cate():
    file_list = [
        '/path/attempt-code/downloads/craw/doc_cat/part0_with_cat.jsonl',
        '/path/attempt-code/downloads/craw/doc_cat/part1_with_cat.jsonl',
        '/path/attempt-code/downloads/craw/doc_cat/part2_with_cat.jsonl',
        '/path/attempt-code/downloads/craw/doc_cat/part3_with_cat.jsonl',
        '/path/attempt-code/downloads/craw/doc_cat/part4_with_cat.jsonl'
    ]
    id2cat = {}
    ...
    for idx, catedict in enumerate(cate_2id_dict):
        ...
        dict_path = '/path/attempt-code/downloads/craw/cate{}2id.json'.format(
            idx+1)
    ...
    with jsonlines.open("/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate.jsonl", 'w') as w:
        for index, row in tqdm(df_task_en.iterrows()):
            product_id = row['product_id'].strip()
            if product_id not in product_id_dict:
                product_id_dict[product_id] = 0
...
def clean_data():
    ...
    input_file = '/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate.jsonl'
    with jsonlines.open("/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate_clean.jsonl", 'w') as w:
...
def make_train_data():
    ...
    """
    f_query_info = open(
        '/path/attempt-code/downloads/amazon_smaller_version/train_query_info.jsonl', 'w', encoding='utf-8')
    f_qrel_info = open(
        '/path/attempt-code/downloads/amazon_smaller_version/train_dev_qrel.txt', 'w', encoding='utf-8')
...
def make_test_data():
    ...
    f_qrel_info = open(
        '/path/attempt-code/downloads/amazon_smaller_version/test_qrel.txt', 'w', encoding='utf-8')
...
def split_train_dev_test():
    f = open('/path/attempt-code/downloads/amazon_smaller_version/train_dev_qrel.txt',
             'r', encoding='utf-8')
    ...
    f_out_dev = open(
        '/path/attempt-code/downloads/amazon_smaller_version/dev_qrel.txt', 'w', encoding='utf-8')
    f_out_train = open(
        '/path/attempt-code/downloads/amazon_smaller_version/train_qrel.txt', 'w', encoding='utf-8')
...
def generate_deatil():
    corpus_file = '/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate_clean.jsonl'
    corpus_dataset = load_dataset('json',  # ‘json'
                                  'default',  # 'default'
                                  data_files={'train': corpus_file})['train']

    train_qrel_file = '/path/attempt-code/downloads/amazon_smaller_version/train_qrel.txt'
    train_qrel_detail_file = '/path/attempt-code/downloads/amazon_smaller_version/train_qrel_detail.txt'
    accord_qrel_generate_detail(
        corpus_dataset, train_qrel_file, train_qrel_detail_file, 'train')

    dev_qrel_file = '/path/attempt-code/downloads/amazon_smaller_version/dev_qrel.txt'
    dev_qrel_detail_file = '/path/attempt-code/downloads/amazon_smaller_version/dev_qrel_detail.txt'
    accord_qrel_generate_detail(
        corpus_dataset, dev_qrel_file, dev_qrel_detail_file, 'dev')

    test_qrel_file = '/path/attempt-code/downloads/amazon_smaller_version/test_qrel.txt'
    test_qrel_detail_file = '/path/attempt-code/downloads/amazon_smaller_version/test_qrel_detail.txt'
    accord_qrel_generate_detail(
...
def make_brand_color_dict(brand_num=18, color_num=7):
    ...
    product_brand_dict2 = {}
    product_color_dict2 = {}
    input_file = '/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate_clean.jsonl'
    with open(input_file, "r+", encoding="utf8") as f:
        print('process: {}'.format(input_file))
        for item in tqdm(jsonlines.Reader(f)):
    ...
    # check_num2count(product_brand_dict2, '/path/attempt-code/downloads/amazon_ori/ori_example_num2brand_num.csv')
    # check_num2count(product_color_dict2, '/path/attempt-code/downloads/amazon_ori/ori_example_num2brand_num.csv')
    ...
    save_dict('/path/attempt-code/downloads/amazon_ori/product_brand2id_' +
              str(brand_num)+'.json', product_brand_dict_14)
    save_dict('/path/attempt-code/downloads/amazon_ori/product_color2id_' +
              str(color_num)+'.json', product_color_dict_6)


def static_cate():
    file = '/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate_clean.jsonl'
    cate_dict = {
        '>=1': 0,
        '>=2': 0,
        '>=3': 0,
...
def generate_category_word_vocab():
    """
    /path/attempt-code/downloads/craw/cate12id.json 59
    /path/attempt-code/downloads/craw/cate22id.json 455
    /path/attempt-code/downloads/craw/cate32id.json 2303
    /path/attempt-code/downloads/craw/cate42id.json 5277
    /path/attempt-code/downloads/craw/cate52id.json 4606
    /path/attempt-code/downloads/craw/cate62id.json 1436
    /path/attempt-code/downloads/craw/cate72id.json 228
    /path/attempt-code/downloads/craw/cate82id.json 29
    /path/attempt-code/downloads/craw/cate92id.json 5
    """
    import re
    regex = r'[^\w\s]|[\s]'
    ...
    # cate_dict_list = {}
    # for i in range(1, 10):
    #     file_dict = '/path/attempt-code/downloads/craw/cate{}2id.json'.format(i)
    ...
    # save_dict('/path/attempt-code/downloads/craw/cate_word_vocab.json', word_to_id)

    # brand_dict =load_dict('/path/attempt-code/downloads/amazon_ori/product_brand2id_13.json')
    ...
    #             if word not in word_to_id:
    #                 word_to_id[word] = len(word_to_id)
    # save_dict('/path/attempt-code/downloads/craw/brand_word_vocab.json', word_to_id)

    # color_dict = load_dict('/path/attempt-code/downloads/amazon_ori/product_color2id_7.json')
    # word_to_id = {}
    ...
    #             if word not in word_to_id:
    #                 word_to_id[word] = len(word_to_id)
    # save_dict('/path/attempt-code/downloads/craw/color_word_vocab.json', word_to_id)
    ...
    save_dict('/path/attempt-code/downloads/craw/cate1-4_word_vocab.json', word_to_id)
...
def generate_whole_aspect_dict():
    ...
    # save_dict('/path/attempt-code/downloads/craw/whole_cate_vocab.json', word_to_id)
    # 12895 -> 12894
    ...
    save_dict('/path/attempt-code/downloads/craw/whole_cate1-4_vocab.json', word_to_id)
...
def generate_category_wordpiece_vocab():
    ...
    /path/attempt-code/downloads/craw/cate92id.json 5
    """
    tokenizer = BertTokenizer.from_pretrained("/path/attempt-code/downloads/bert-base-uncased")
    import re
    regex = r'[^\w\s]|[\s]'
    ...
    # save_dict('/path/attempt-code/downloads/craw/cate_wordpiece_vocab.json', word_to_id)
    ...
    #                 if w not in word_to_id:
    #                     word_to_id[w] = len(word_to_id)
    # save_dict('/path/attempt-code/downloads/craw/brand_wordpiece_vocab.json', word_to_id)
    ...
    #                 if w not in word_to_id:
    #                     word_to_id[w] = len(word_to_id)
    # save_dict('/path/attempt-code/downloads/craw/color_wordpiece_vocab.json', word_to_id)
    ...
    save_dict('/path/attempt-code/downloads/craw/cate1-4_wordpiece_vocab.json', word_to_id)
...
#     "/path/attempt-code/downloads/bert-base-uncased")
# tokenize_for_qrel_detail(tokenizer, '/path/attempt-code/downloads/amazon_smaller_version/dev_qrel_detail.txt',
#                          '/path/attempt-code/downloads/amazon_smaller_version/tokenize/dev_qrel_detail.txt')
# tokenize_for_qrel_detail(tokenizer, '/path/attempt-code/downloads/amazon_smaller_version/test_qrel_detail.txt',
#                          '/path/attempt-code/downloads/amazon_smaller_version/tokenize/test_qrel_detail.txt')
# tokenize_for_qrel_detail(tokenizer, '/path/attempt-code/downloads/amazon_smaller_version/train_qrel_detail.txt',
#                          '/path/attempt-code/downloads/amazon_smaller_version/tokenize/train_qrel_detail.txt')

# tokenize_for_corpus(tokenizer, '/path/attempt-code/downloads/amazon_smaller_version/amazon_corpus_with_cate_clean.jsonl',
#                     '/path/attempt-code/downloads/amazon_smaller_version/tokenize/amazon_corpus_with_cate_clean.jsonl')
